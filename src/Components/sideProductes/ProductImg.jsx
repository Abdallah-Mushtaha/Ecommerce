...
export default function ProductImg({ items, Loading }) {
  ...
  return (
    <div className="imgDev flex flex-col gap-2 px-5 sm:px-0">
      ...
      {/* Small imgs */}
      <div className="small min-w-screen sm:w-40 h-auto flex flex-row flex-wrap sm:flex-nowrap gap-2 cursor-pointer">
        {items.images.map((img, index) =>
          Loading === true ? (
            <div
              key={index}
              className="max-w-sm p-4 w-full rounded animate-pulse md:p-6"
            >
              {/* when Image Still loading */}
              <div className="flex items-center justify-center size-24 mb-4 bg-gray-300 rounded-full dark:bg-gray-400">
                <svg
                  viewBox="0 0 16 20"
                  fill="currentColor"
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <img
              ...
              className="w-20 h-20 sm:w-auto sm:h-auto object-cover relative z-10"
              alt={items.title}
              onClick={() => {
                setImgIndex(index);
              }}
